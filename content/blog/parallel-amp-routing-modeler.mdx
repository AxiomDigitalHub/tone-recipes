...
title: >-
  ...
  One Amp Can't
seo_title: "Parallel Amp Routing: When Two Amps Beat One"
description: >-
  Two amp models in parallel, summed to mono, build one tone from two amp
  characters. When the technique earns its place, how to wire it on Helix and
  Quad Cortex, and how to align the paths.
date: '2026-04-28'
updated: '2026-08-03'
...
takeaways:
  - >-
    Parallel amp routing feeds one dry signal into two amp models and sums both
    to mono. It is not a stereo split and not a wet/dry rig — the goal is one
    tone built from two complementary amp characters.
  - >-
    The technique fails when both amps are left flat, because they fill the
    same upper midrange and the sum smears. EQ each amp to own a different band
    and the two interlock instead of competing.
  - >-
    Test the mono fold-down before the patch ships. A parallel patch that only
    holds together in stereo will collapse the moment a venue's PA sums it.
  - >-
    Path misalignment is real and measurable, not a matter of taste. Two amp
    and cab chains rarely have identical processing latency, and a few samples
    of offset comb-filters the sum. Measure the offset in a DAW rather than
    guessing at a delay value.
  - >-
    Put reverb and delay after the sum, not on each path. Time-based effects on
    both paths double the CPU cost and add a second phase relationship to keep
    track of.
faq:
  - q: What is the difference between parallel amps and a stereo amp setup?
    a: >-
      Parallel amps sum two amp models into one mono output, so both amps
      contribute to a single tone. A stereo amp setup keeps the two amps panned
      hard left and right to create a wide image. Use parallel for tonal
      blending and stereo for spatial width.
  - q: Will parallel amp routing work in a live mono PA?
    a: >-
      Yes, if you check the mono fold-down before you ship the patch. A patch
      that sounds full in stereo can collapse in mono from phase issues. Solo
      the parallel section, sum to mono, and listen for comb filtering or a
      thinned low end before relying on it live.
  - q: Does parallel amp routing use more CPU on a modeler?
    a: >-
      Yes. Two amp blocks plus two cab or IR blocks plus the EQ on each path
      roughly doubles the load of a single-amp patch. On Helix that matters at
      higher block counts. Check the CPU meter before adding modulation and
      reverb on top.
  - q: How do I find the right time alignment between two parallel amp paths?
    a: >-
      Measure it rather than guessing. Record each path solo to its own track,
      zoom in on the same pick attack in both, and read the offset in samples.
      Divide by your sample rate to get milliseconds, then delay the earlier
      path by that amount. A null test confirms it.
  - q: Should reverb and delay go on each amp path or after the sum?
    a: >-
      After the sum, almost always. Time-based effects on each path double the
      CPU cost and introduce a second set of phase relationships between the
      two wet signals. Sum the amps first, then process the combined signal.
  - q: Is parallel amp routing used on records?
    a: >-
      Extensively. Modern fusion and prog production uses it routinely, and
      players have done the same thing with physical amp rigs for decades by
      splitting into two heads and blending the miked signals. The modeler
      version only makes the routing easier to build.
readingTime: 10
image: /images/blog/parallel-amp-routing-modeler.jpg
image_alt: >-
  A modeler screen showing two parallel signal path rows, each with its own amp
  and cab block, merging into a single mixer block before the output
---
...
## How to Find the Alignment Offset Instead of Guessing

"Try 1-3 ms and see what sounds better" is the advice everywhere, and it is unsatisfying for a good reason: the correct value is not a matter of taste. There is a specific offset between the two paths, it is a fixed number of samples, and you can read it directly rather than hunting for it by ear.

The offset exists because the two paths are not doing equivalent amounts of work. Amp models differ in internal processing latency, cab blocks and IR blocks do not cost the same, and an EQ block on one path and not the other adds its own small delay. None of these are large individually. They do not have to be — at 48 kHz, a 48-sample difference is 1 ms, and 1 ms of offset between two correlated full-range signals puts a comb notch at 500 Hz and every odd multiple above it. That is the hollowness people describe and then try to EQ away.

**The measurement, in five steps:**

1. **Record each path solo** into its own DAW track, same performance, same take if your unit can print both simultaneously. A single hard pick attack on a low string is the ideal test signal — you need one sharp transient, not a riff.
2. **Zoom all the way in** on that transient in both tracks, to the sample level.
3. **Read the offset in samples** between the two leading edges. Most DAWs will show the sample position of the cursor directly.
4. **Divide by your sample rate** to convert to milliseconds. At 48 kHz, 96 samples is 2 ms.
5. **Delay the earlier path by that amount** using a delay or time-align block set to zero feedback and 100% wet.

**Confirm with a null test.** Flip the polarity on one path and play. If the alignment is correct and the two signals are otherwise similar, the sum should drop dramatically in level — the deeper the null, the tighter the alignment. Then flip the polarity back. This takes about a minute and it converts "sounds a bit better maybe" into a number you can verify.

Two caveats worth knowing before you spend time on this. The first is that a perfect null is not the goal and is not achievable, because the two amps are deliberately EQ'd differently and are producing genuinely different content — a deep null would mean the two paths are redundant. You are aligning the transient, not making the signals identical. The second is that the offset is a property of the specific blocks on each path, so it changes the moment you swap an amp model, change a cab, or add a block to one side. Re-measure after any structural edit; do not carry a delay value over from a previous patch.

If you would rather not measure, the fallback that costs nothing is to keep the two paths structurally symmetrical — same number of blocks, same block types in the same order, different settings. Symmetry does not guarantee alignment, but it removes the largest and most avoidable source of it.
...
For more on signal chain architecture, see [our stereo signal chain architecture guide](/blog/stereo-signal-chain-architecture) and [the Helix global EQ guide](/blog/modeler-global-eq-guide). The same split-and-merge topology applied to reverbs — along with the correlation problem it introduces — is covered in [parallel reverb routing](/blog/parallel-reverb-routing). And if you have hit the limit of what split paths can do, note that both are forward-only: producing anything that regenerates requires [a physical send-to-return loop](/blog/modeler-send-return-loop-tricks-beyond-shimmer).
