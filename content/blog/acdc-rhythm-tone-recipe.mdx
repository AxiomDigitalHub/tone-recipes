...
seo_title: "AC/DC Rhythm Tone Recipe: Angus Young's Settings"
...
readingTime: 12
image: "/images/blog/acdc-rhythm-tone-recipe.jpg"
image_alt: "A Marshall-style stack and an SG-style guitar set up for an AC/DC rhythm tone, with amp settings highlighted"
---
...
The reason is that AC/DC's sound is built on the physics of loud amplifiers, not on clever settings or exotic gear. [Angus Young](/artist/angus-young)'s tone comes from a Marshall Super Lead being driven hard enough that the output transformers are working. The power section is saturating, the speakers are compressing, and the whole rig is operating at the upper edge of its design envelope. You can't replicate that behavior by turning the preamp gain up. You replicate it by running the amp loud.
...
Angus Young plays [Gibson SG Standards](/gear/gibson-sg-standard). He's played them since the early days and hasn't meaningfully deviated. The SGs he uses are loaded with PAF-style humbuckers. Low-to-medium output, clear note separation, enough output to push a Marshall into breakup without requiring a high-gain circuit.

The SG's body construction matters too. It's lighter than a Les Paul, and the resonance hits different. More mids, quicker attack, less low-end weight. That forward-mid character is what feeds a Marshall the right signal.
...
Angus Young used a [1959 Super Lead Plexi](/gear/marshall-super-lead-1959). That's the 100-watt Marshall that most players mean when they say "Marshall Plexi." Malcolm used a Super Bass, which is a similar circuit with a slightly different EQ voicing in the bass frequencies. Both amps were run hard.

For Back in Black (1980), the studio amps were modified Plexis with tighter low-end. The specifics are debated. What matters is the sound: a Plexi-circuit Marshall, run hard, doing its job.
...
The [complete guide to guitar amp types](/blog/complete-guide-guitar-amp-types) covers why Plexi-circuit Marshalls behave differently from master-volume designs. The short version: the gain staging is fundamentally different when there's no master volume to limit the power section.
...
The mids-scooped, bass-and-treble-up setting is the "Marshall smile." It's called that because the EQ curve looks like a smile on a flat graphic EQ. It's the British rock default for a reason. On a Plexi being run loud, the mids that appear scooped on the control panel come back in the power section response. The amp fills the midrange back in as the output section saturates.
...
| Cab | 4x12 with Greenback IRs | G12M or G12H30, not V30s (too mid-forward) |
...
<SaveThisTone recipeSlug="angus-young-back-in-black-rhythm" />
