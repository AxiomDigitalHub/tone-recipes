...
seo_title: "LR Baggs Anthem vs. Fishman vs. K&K Pure Mini"
description: >-
  Three of the most-installed hybrid acoustic pickup systems compared on real
  guitars: pickup character, install difficulty, feedback resistance, and how
  much EQ repair each one needs downstream.
date: '2026-04-28'
updated: '2026-07-28'
author: Elena Ruiz
...
takeaways:
  - >-
    All three systems pair an undersaddle or soundboard contact pickup with a
    second sensor; what separates them is whether that second sensor is a
    microphone, and a microphone is what buys you body resonance and costs you
    feedback resistance.
  - >-
    The LR Baggs Anthem is the pick if you amplify at any real volume, the
    Fishman Matrix Infinity Mic Blend is the value choice for home and
    mid-volume use, and the K&K Pure Mini is the right answer for quiet
    recording and smaller-bodied guitars.
  - >-
    Install difficulty tracks price almost exactly: the K&K is DIY-approachable,
    the Fishman needs saddle work, and the Anthem is a luthier job with $80-$120
    of labor on top of the system.
  - >-
    A mic-blend system reduces how much subtractive EQ you need downstream but
    never eliminates it — every undersaddle element in these systems still
    produces the 200-350 Hz box and the 2.5-4 kHz quack.
  - >-
    Match the system to the room you actually play in, not the loudest room you
    might someday play in. An Anthem in a bedroom is overkill; a K&K on a loud
    stage will feed back.
faq:
  - q: Can I install any of these acoustic pickup systems myself?
    a: >-
      The K&K Pure Mini is approachable for a patient DIYer with no luthier
      experience; just careful epoxy work on the bridge plate. The Fishman
      Matrix Infinity Mic Blend is doable if you're comfortable with saddle
      work. The LR Baggs Anthem is a luthier job — the bridge plate routing and
      the internal microphone mounting require skills most home players don't
      have. If unsure, pay the install fee.
  - q: Will any of these work for a high-volume rock band setting?
    a: >-
      None of them are built for that use case. At rock-band stage volumes
      you'll want a magnetic soundhole pickup like the LR Baggs M1 Active, or a
      piezo-only system without the microphone component. The hybrid systems
      here are designed for solo acoustic, small ensemble, and
      amplified-but-not-loud contexts.
  - q: What's the difference between the Matrix Infinity and the Matrix Infinity Mic Blend?
    a: >-
      The base Matrix Infinity is the undersaddle pickup alone. The Mic Blend
      version adds the soundhole-mounted condenser mic and the blend control. If
      you only need an undersaddle pickup and don't want the mic, the base
      version is around $180. The blend version adds the mic capability for
      about $70 more.
  - q: Do these pickup systems work on any acoustic guitar?
    a: >-
      Most acoustic guitars, yes, but the install requires a flat, accessible
      bridge plate (most flat-tops have this) and a saddle slot of standard
      depth (most have this). Archtop guitars and unusual bridge designs may
      require different solutions. Check with a luthier before ordering if your
      guitar is unusual.
  - q: Is the Taylor ES2 system from the factory comparable to these?
    a: >-
      The Taylor ES2 is a behind-the-saddle design (not undersaddle) with three
      sensors. It's good — better than most older Taylor systems — but it's
      purely contact-based with no microphone component, so it sits closer to a
      refined undersaddle than to a hybrid. If you have an ES2 and want the
      body-resonance capture a mic adds, an Anthem or Matrix Infinity Mic Blend
      is the upgrade. The K&K Pure Mini isn't; that's a different architecture
      targeting a different sound.
  - q: Does a better pickup mean I need less EQ?
    a: >-
      Less, not none. Blending in a microphone restores body resonance that no
      EQ curve can create, which is the part a pickup upgrade genuinely fixes.
      But every system here still has an undersaddle or contact element in the
      blend, so the 200-350 Hz box and the 2.5-4 kHz quack are still in the
      signal and still want cutting.
image: /images/blog/hybrid-acoustic-pickup-comparison.jpg
image_alt: >-
  Three acoustic guitar pickup systems laid out on a workbench beside an opened
  acoustic guitar soundhole, with an undersaddle element, a soundhole
  microphone, and soundboard transducer discs visible
---
...
**What it sounds like:** The Matrix Infinity by itself is one of the cleaner undersaddle pickups on the market, with less of the harsh piezo "quack" than older systems. Adding the mic blend gives the sound depth and body. With the mic at 50%, a Taylor 214ce sounded fuller than the same guitar with just the stock ES2. The mic captures more of the high-frequency air than the Anthem's TRU-MIC does, which works well for fingerstyle but can get bright on aggressive strumming.
...
## How Much EQ Repair Each One Needs

Here's the part I wish someone had put in a table before I started shopping, because it reframes the whole price question. Every system here still has a contact element in the blend, which means every system still produces some amount of the two classic undersaddle problems: the boxy honk around 200-350 Hz and the brittle quack between 2.5 and 4 kHz. What the money buys is *how much* of that you have to fix downstream, not whether you have to fix it.

| System | Low-mid box (200-350 Hz) | Quack (2.5-4 kHz) | Body resonance |
|---|---|---|---|
| **LR Baggs Anthem** | Mild — the internal mic fills in around it | Mild at low mic blend, minimal at high blend | Genuinely present from the mic |
| **Fishman Matrix Infinity Mic Blend** | Moderate — the mic helps but sits further from the top | Moderate; still wants a cut | Partial, blend-dependent |
| **K&K Pure Mini** | Low — soundboard transducers don't have the saddle-pressure signature | Low; this is the K&K's main advantage | Some, from the top itself |

Two things fall out of that. First, the K&K's reputation for sounding natural isn't marketing — it isn't an undersaddle pickup at all, so the quack that defines the piezo sound is largely absent from the source. Its weaknesses are feedback resistance and output level, not tone color. Second, the Anthem's price is buying body resonance specifically, and body resonance is the one thing on this list that EQ genuinely cannot manufacture. That's a real reason to spend the money, and it's a different reason than "it sounds better."

If you're deciding between spending on a better pickup and spending the time to EQ the one you have, that distinction is the whole decision. A tone problem — honk, quack, brittleness — is a cut or two away and costs nothing. A missing-body problem is a hardware problem, or an [acoustic IR](/blog/acoustic-ir-vs-subtractive-eq-piezo-real) problem, and no EQ curve will solve it. Figure out which one you actually have before you spend $300.

Whatever you land on, the signal path after the pickup matters as much as the pickup: the [acoustic-through-a-modeler walkthrough](/blog/acoustic-guitar-through-modeler-worship-di) covers the DI routing, the compressor, and the feedback notch in order.
