...
seo_title: "MXR Carbon Copy vs. Boss DM-2W: Two BBD Delays"
description: >-
  Both the MXR Carbon Copy and Boss DM-2W use the same BBD chip, but one is a
  dark ambient washer and the other a clean, versatile rhythm delay. Here's
  which to buy for your music.
date: '2026-04-28'
updated: '2026-06-26'
...
image_alt: >-
  An MXR Carbon Copy and a Boss DM-2W Waza Craft analog delay pedal side by
  side on a pedalboard, both built around the MN3005 bucket-brigade chip
takeaways:
  - >-
    Both pedals run the same MN3005-family BBD chip, so the difference is the
    circuit around it, not the chip — the Carbon Copy is dark and modulated, the
    DM-2W is clean and transparent.
  - >-
    Buy the Carbon Copy for always-on ambient atmosphere and washed-out repeats
    with character; its repeats darken fast into a low-mid smudge that sits
    behind the dry note.
  - >-
    Buy the DM-2W for slap-back, tight dotted-eighth rhythm in a mix, and any
    time the delay needs to disappear — it holds high end longer and has a
    lower noise floor.
  - >-
    Only the DM-2W has an expression input (sweep delay time with a foot pedal)
    and dual standard/custom voicings; the Carbon Copy has every-setting
    character but no expression control.
  - >-
    The Carbon Copy self-oscillates musically rather than chaotically, which
    makes it the better of the two for riding the feedback knob as a sound-
    design tool. Both are around $150 street and built to last decades.
faq:
  - q: Which one is louder when it's off?
    a: >-
      Both have buffered bypass and add a small amount of buffer-stage character
      to your dry signal even when bypassed. The DM-2W's buffer is typical Boss
      voicing (slightly bright); the Carbon Copy's is more transparent. If you
      have a long pedalboard run before the delay, both buffers are net
      positives.
  - q: Can either one be powered with a daisy chain power supply?
    a: >-
      The Carbon Copy can; its current draw is about 25 mA, which is low. The
      DM-2W draws about 75 mA, high enough that you'll want a regulated isolated
      supply if you're running other digital pedals on the same daisy chain. See
      the pedalboard power supply isolation guide for the diagnostic order.
  - q: Is the DM-2W really the same as a vintage 1981 DM-2?
    a: >-
      In standard mode, very close. The DM-2W uses the same MN3005 chip family,
      the same circuit topology, and the same component values where possible.
      The voicing is faithful enough that side-by-side blind tests are
      inconclusive. The custom mode adds capabilities the original DM-2 didn't
      have.
  - q: Why does the Carbon Copy have those internal trim pots?
    a: >-
      The two internal trim pots adjust the rate and width of the modulation.
      From the factory they're set to a subtle baseline. Open the pedal and turn
      them clockwise with a tiny screwdriver to push the modulation toward
      chorus-like territory. It's one of the under-documented features that
      makes the Carbon Copy more flexible than it looks.
  - q: What's the cheapest decent BBD delay if I can't afford either of these?
    a: >-
      The Way Huge Aqua-Puss MkIII at around $100 is genuinely close to both,
      with a shorter max delay time but the same MN3005-family chip. The MXR
      Carbon Copy Mini at around $130 is a smaller Carbon Copy with modulation
      always on but no internal trim pots. Both are real BBD pedals, not
      PT2399-based imposters.
---
...
**Always-on ambient atmosphere.** The dark, slightly modulated character of the Carbon Copy makes it the kind of pedal you can leave on for an entire set and the audience won't notice it as a "delay." They'll hear it as part of your overall guitar sound. This is how you use it for shoegaze, dream-pop, slowcore, and post-rock; anywhere the delay should be the cushion, not the rhythm element.

**Long, washed-out repeats with character.** The Carbon Copy starts to oscillate at higher regen settings in a way that's musical rather than chaotic. The Mod toggle adds a wobble that imitates the wow-and-flutter of a real tape delay. Push the regen past 2 o'clock and you get an evolving texture that feeds back on itself in the way classic ambient guitar records use. That musical-rather-than-chaotic oscillation is also why the Carbon Copy is the better of the two for [riding the feedback knob as an instrument](/blog/controlled-delay-self-oscillation-ambient-noise-instrument) — its BBD path darkens and degrades as the howl builds, so the runaway turns into a usable drone instead of an ice-pick squeal.
