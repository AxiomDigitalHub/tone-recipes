...
seo_title: "BBD vs. PT2399: How the Analog Delay Chips Sound"
description: "Analog delay pedals split into two chip camps: the bucket-brigade BBD is darker and more compressed with shorter times, while the PT2399 is brighter, quieter, and longer — but not strictly analog."
date: '2026-04-27'
...
takeaways:
  - >-
    Both chips get sold as "analog delay," but only the bucket-brigade device
    (BBD) keeps the signal analog end to end; the PT2399 converts to digital
    internally and back, so the label is about character, not signal path.
  - >-
    The BBD rolls off treble on every repeat, so echoes darken as they decay —
    great for slap-back and short ambient tails, but it maxes out around 600 ms.
  - >-
    The PT2399 keeps repeats cleaner and reaches past a second, which makes it
    the better pick for audible dotted-eighth rhythm work and long ambient
    washes.
  - >-
    The surprise across a month of A/B recordings: the cheaper PT2399 covers
    more real applications than the BBD, which is a specialist, not a
    general-purpose delay.
  - >-
    You can recreate the BBD's darkening decay on any clean digital delay by
    putting a low-pass filter in an external feedback loop, so the character is a
    routing trick, not just a chip.
faq:
  - q: Is a BBD pedal worth more than a PT2399 pedal?
    a: >-
      Used market values reflect the sourcing realities — BBD chips (MN3005,
      MN3007) are out of production and getting harder to source, which has
      pushed prices for both vintage BBD pedals and current BBD-based designs
      higher. The DM-2W and Carbon Copy are $150 to $200 retail in part because
      the chips themselves are expensive. PT2399 chips remain in production and
      cost less than a dollar each, which is why PT2399 pedals start at $50. The
      price difference reflects component cost more than tonal superiority.
  - q: Will the BBD chip eventually be unavailable for new production?
    a: >-
      It's a real concern. Panasonic discontinued the MN3005 in the early 2000s,
      and current BBD pedals use either remaining stock or alternative chips like
      the V3205 (a Cool Audio reissue that's chemically similar but not
      identical). The Boss DM-2W uses the V3205. The MXR Carbon Copy uses what
      MXR has stockpiled plus the V3205 in newer production. If you want a
      BBD-character pedal for the next 20 years, buy a current BBD-based pedal
      now rather than assuming you'll be able to buy one in five years.
  - q: Can a digital delay sound like a BBD?
    a: >-
      Modern modeling can come close. The Strymon DIG, the Helix Vintage Digital
      model, and the Walrus Audio Slö all attempt to model BBD behavior. They get
      the surface character (high-frequency rolloff per repeat, modulation
      warble, soft saturation) but they don't fully capture the way a real BBD
      chain compresses and degrades the signal in motion. You can also chase the
      darkening decay directly with a low-pass filter in a delay's external
      feedback loop, which recreates the per-repeat rolloff on a clean digital
      delay.
  - q: Why does the PT2399 sound "analog" if it's digital internally?
    a: >-
      Because the digital section is intentionally bandwidth-limited and the
      analog input/output stages add the softness most players associate with
      analog gear. The chip's internal sample rate is around 44 kHz with limited
      bit depth, so the high-frequency content is reduced and quantization
      artifacts sit at a level that reads as character rather than harshness.
      Combined with the analog filtering most pedals add around the chip, the
      result feels organic even though the signal path includes digital
      processing. The label is about how it sounds, not the signal path.
  - q: If I can only afford one delay pedal, which chip should I buy?
    a: >-
      If your playing leans rhythmic (dotted-eighth, half-tempo delays, longer
      atmospheric work), buy a PT2399-based pedal — the Mooer Reecho or Walrus
      Audio ARP-87 are both excellent. If your playing leans toward slap-back,
      vintage character, and short ambient work, buy a BBD pedal — the MXR Carbon
      Copy is the value pick at $150. If you don't know your bias yet, buy the
      PT2399 first because it covers more applications and is significantly
      cheaper. Your second delay can be the BBD when you find the use case where
...
Most pedals marketed as "analog delay" fall into two camps based on which chip they use. The BBD and the PT2399 both get called analog, but they're different circuits with different sonic signatures. The bucket-brigade chip in a Boss DM-2W and the PT2399 in basically every budget analog delay made in the last 15 years both produce a warm, slightly dark character compared to a Strymon Timeline or a Helix delay block. But they aren't the same thing, and the players who know the difference also know which one to reach for in which context.
...
## The Character Is a Routing Trick, Not Just a Chip

Here's the part that reframes the whole comparison once you've heard both chips. The thing that makes a BBD sound like a BBD — repeats that get darker as they decay — isn't locked inside the silicon. It's what happens when the delayed signal passes through a low-pass filter *every time it recirculates*. The BBD does that automatically because the chip itself is a low-pass filter that the signal re-crosses on every trip around the feedback path.

Which means you can recreate it on a clean digital delay that has no BBD character at all, as long as the delay exposes an external feedback loop. Patch a low-pass filter into that loop, roll the cutoff down to around 2–3 kHz, and each repeat loses a slice of top end the previous one still had — the same darkening decay the BBD gives you, on a delay with a pristine first repeat and no fixed 600 ms ceiling. I've [walked through the external-feedback-loop technique in full](/blog/delay-external-feedback-loop-processing-repeats) — filter, pitch, and drive in the loop — but the filter case is exactly the BBD-in-software move. It reframes what you're actually buying with a BBD pedal: not a magic tone, but a filter permanently wired into the feedback path. Useful to know, because it means the choice below is about convenience and ceiling, not about a character only one chip can produce.
