...
takeaways:
  ...
  - >-
    When delay and reverb share one tempo grid, the whole ambient bed breathes in
    rhythm; when they don't, the reverb tail blurs across the beat and the part
    sounds washed-out rather than spacious.
  - >-
    Set the reverb decay to clear roughly within a bar — about one bar of tail on
    slow songs, half a bar on faster ones — so the space resets before the next
    chord instead of piling up into mud.
  - >-
...
faq:
  ...
  - q: Why does syncing predelay to tempo matter?
    a: >-
      Because an un-synced reverb bloom lands in the cracks between beats, so the
      ambience fights the groove instead of riding it. When predelay is locked to
      a subdivision, the reverb arrives at a musical moment and the whole wash
      pulses with the song. It's the difference between spacious and just
      washed-out.
  - q: How long should the reverb decay be?
    a: >-
      Tie it to the bar. Aim for the tail to clear roughly within one bar on a
      slow song and about half a bar on a faster one, so the reverb resets before
      the next chord lands. At 70 BPM a bar is around 3.4 seconds; at 130 BPM it's
      about 1.8. Longer tails pile up into mud; shorter ones keep the part
      readable.
