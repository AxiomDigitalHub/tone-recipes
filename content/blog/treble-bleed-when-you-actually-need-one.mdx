...
The resistance the pot presents is at its highest when the wiper sits at the resistive midpoint of the track — a quarter of the pot's total value. For a 250k pot that is 62.5k; for a 500k pot, 125k. (The pot value has a second, separate job as a load on the pickup at full volume, which pulls the opposite way; [what the pot value actually loads](/blog/volume-pot-value-250k-vs-500k-what-it-loads) is the companion piece to this one.) Put that against the capacitance downstream and you get a corner frequency for the rolloff you are hearing.
