...
takeaways:
  - >-
    ...
    sounds pressed against the glass. A short reverb after the IR puts the guitar
    back into a space and closes that gap.
  - >-
...
faq:
  - q: Why does my cab IR sound dry and lifeless?
    a: >-
      ...
      floor. An IR gives you the speaker sound with no reflections, so the guitar
      sounds close-mic'd and pressed against the glass. Adding a short reverb
      after the IR puts those reflections back and the guitar moves into a space.
  - q: Should the reverb go before or after the cab IR?
    a: >-
      ...
      reality is amp, then cab IR, then reverb. Putting reverb before the IR runs
      the reverb tail through the speaker model, which smears it and sounds
  ...
  - q: What reverb settings close the gap on a dry IR?
    a: >-
      ...
      hear the reverb as an effect, it is probably too loud for closing the space
      gap, though a louder, longer setting is a fine creative choice for ambient
      parts.
  - q: Can a pedal like the BlueSky run in a recording chain?
    a: >-
      Yes. You can run it in front of the interface, in an effects loop, or on an
      insert via a reamp setup. The catch is that a hardware reverb is committed
      to the recording unless you print it to a separate track, while a plugin
      stays editable forever. Many people track dry, then choose between a
      hardware reverb on a reamp pass and a plugin in the box afterward.
---
