...
seo_title: "V/Hz: Running a 60Hz Vintage Amp on 50Hz Mains"
description: >-
...
image_height: 1024
image_alt: >-
  A tube amplifier chassis on a bench with a multimeter reading mains voltage
  and a step-down voltage converter connected to it
takeaways:
  - >-
    Transformer flux density is proportional to voltage divided by frequency, so
    a 120V/60Hz amp run at 120V on 50Hz mains sees flux density rise about 20% —
    into saturation territory.
  - >-
    To hold flux density constant on 50Hz, drop the voltage to the same ratio:
    120V times 50/60 equals 100V, which is 83% of nominal.
  - >-
  ...
  - >-
    100V at 50Hz is, not coincidentally, almost exactly the Japanese mains spec,
    so a 60Hz amp is already proven to tolerate that operating point.
  - >-
    The cost of running at 83% is a browner, softer, earlier-breakup tone with
    less clean headroom — often desirable, rarely a problem, and far cheaper
    than a re-wound transformer.
faq:
  - q: What is the volts-per-hertz rule for a tube amp transformer?
    a: >-
      Transformer core flux density is proportional to applied voltage divided
      by frequency (V/Hz). A power transformer is designed for a specific V/Hz
      ratio — 120V at 60Hz, for example. Keep that ratio constant and the core
      stays in its safe operating range; raise it and the core saturates.
  - q: Why does 50Hz mains stress a 60Hz amp?
    a: >-
      At the same voltage, lowering the frequency from 60Hz to 50Hz raises the
      V/Hz ratio by 60/50, or about 20%. That pushes the transformer core toward
      saturation, where magnetizing current climbs steeply and the core heats
      up. The frequency, not just the voltage, is doing the damage.
  - q: What voltage should I run a 120V amp at on 50Hz mains?
    a: >-
      About 100V. Multiply nominal voltage by the frequency ratio: 120V times
      50/60 equals 100V, which holds the transformer's flux density at its 60Hz
      design value. That's 83% of nominal — the amp will sag and lose some
      headroom, but the transformer runs at its intended flux.
  - q: Is under-volting a tube amp safe?
    a: >-
      For the transformer, yes — it lowers flux density and runs the core
      cooler. The trade is less B+ voltage, so less clean headroom and earlier,
      softer breakup, plus slightly under-heated tubes if you go far below
      nominal. It is the safe direction. Over-voltage and over-flux are what
  ...
  - q: Does a voltage converter fix the 50Hz frequency problem?
    a: >-
      ...
      without being a true frequency converter, which is rare and expensive. But
      by stepping the voltage down to about 100V it corrects the V/Hz ratio,
      which is the part that actually stresses the transformer. That's the
      practical fix.
---
