...
image_alt: >-
  ...
  isolation transformer box visible, illustrating ground-loop hum troubleshooting
takeaways:
  - >-
    Wet/dry/wet hum is almost always a ground loop — two or more amps connected
    by signal cables while also sharing a power ground, forming a loop that picks
    up mains hum at 60Hz (or 50Hz abroad).
  - >-
  ...
  - >-
    Match the fix to the symptom: hum that changes when you touch strings or move
    a cable is a signal loop (lift first); hum that's constant and only dies with
    full isolation needs a transformer.
faq:
  - q: Why does my wet/dry/wet rig hum?
    a: >-
      ...
      amp) and also share a ground through their power cords. That creates a loop
      of conductor that acts like an antenna for mains hum, so you hear a steady
      60Hz buzz (50Hz outside the Americas) that gets worse the more amps you add.
  - q: Should I use a ground lift or a transformer to fix amp hum?
    a: >-
      ...
      signal-side loop and often kills the hum outright. If the lift only reduces
      the hum, or your boxes have no lift switch, step up to an audio isolation
      transformer, which fully breaks the loop and is the dependable fix for
      stubborn cases.
  - q: Is it safe to lift the ground to stop hum?
    a: >-
      It is safe to lift a signal ground — that's what the ground-lift switch on
      a DI or transformer isolator does, and it doesn't affect electrical safety.
      It is not safe to lift the AC mains ground. Never use a cheater plug or clip
      the third prong on a power cord to chase hum; that defeats the safety earth
      and can leave a live chassis. Fix hum on the signal side only.
  - q: What does a transformer isolator actually do?
    a: >-
      An audio isolation transformer passes the signal magnetically across two
      coils with no direct wire connection between input and output. Because there
      is no continuous conductor, the ground loop is physically broken — the hum
      current has no path to flow. It's the most complete fix for ground-loop hum
      and works even when a simple ground lift doesn't.
