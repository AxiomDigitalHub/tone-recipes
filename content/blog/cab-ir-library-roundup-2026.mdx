...
title: >-
  ...
  Options
seo_title: "Cab IR Library Roundup: Ownhammer, York, ML Sound"
description: >-
  Ownhammer, York Audio, ML Sound Lab, and the free packs each voice a cab
  differently through mic placement, room, and post-processing. How to choose
  the IR library that fits your rig.
date: '2026-05-05'
updated: '2026-07-10'
...
image_alt: >-
  A laptop showing a modeler's IR loader screen beside a 4x12 guitar cabinet,
  representing a comparison of cab impulse response libraries
takeaways:
  - >-
    Which cab IR library you choose is often a bigger tonal decision than which
    amp model you pick. Swapping libraries changes the room, the mic, and the
    engineer's taste all at once, not just the speaker.
  - >-
    Ownhammer is dry and surgical for slotting into a dense mix; York Audio is
    warm and three-dimensional for live FRFR; ML Sound Lab is bright and
    present-mid, engineered to cut through modern-metal layering.
  - >-
    Wilkinson Audio's free God's Cab pack (700+ Mesa V30 4x12 captures) and
    Celestion's free sampler IR cover roughly 70 percent of real use cases for
    nothing. Try those before spending a dollar.
  - >-
    Audition IRs in context and on the playback system you'll actually use, then
    pick the one that needs the least EQ to sit right — not the prettiest one in
    isolation.
  - >-
    CelestionPlus's consistent capture conditions make it the cleanest choice
    for true A/B speaker comparisons, because only the speaker changes between
    captures.
faq:
  - q: 'Do I need separate IR libraries for different amp models?'
    a: >-
      No, but you do need IRs that match the cab the amp is meant to be played
      through. A Marshall amp model wants a 4x12 Greenback or V30 IR; a Fender
      model wants a 1x12 or 2x12 with a Jensen-style speaker; a Vox AC30 model
      wants a 2x12 Blue Alnico IR. Within those categories, any of the libraries
      here work — the choice is tonal preference, not amp model.
  - q: 'Why does my IR sound different on different playback systems?'
    a: >-
      Because each playback system colors the IR differently. Studio monitors
      are flat and reveal IR character honestly. FRFR cabs add their own voicing
      on top of the IR. Headphones emphasize close-mic detail and minimize room
      contribution. Pick the IR on the system you'll actually use: monitors for
      studio work, FRFR for live, headphones for headphone tracking.
  - q: 'Should I use cab IRs with a voiced powered cab like the Friedman ASM-12?'
    a: >-
      Usually no. A powered guitar cab like the ASM-12 is voiced like a guitar
      cab itself, so adding a cab IR on top stacks two cab curves and the tone
      gets boxy. Either bypass the cab IR when running into a voiced FRFR, or use
      a flat-response FRFR like the Atomic CLR with cab IRs enabled. Don't stack
      two cab voicings.
  - q: 'Are paid IR libraries worth it over the free God''s Cab pack?'
    a: >-
      For most players the free pack covers 70 to 80 percent of use cases. The
      paid libraries earn their price when you need a specific cab the free pack
      lacks, more mic positions for fine-tuning, or a tonal character (warm York
      Audio, present-mid ML) the free pack doesn't offer. Try the free pack
      first; upgrade only when you can name the limitation.
  - q: 'Can I mix IRs from different libraries in the same preset?'
    a: >-
      Yes, and most modelers let you blend two cab IRs in parallel. Pairing a
      tight close-mic'd IR with a more dimensional room-mic IR can give a fuller
      composite than either alone. Start around a 70/30 close-to-room blend and
      adjust by ear — just watch the timing, because two captures at different
      distances can comb-filter against each other the same way two real mics do.
---

> **Quick read:** Cab impulse responses are not interchangeable. Each library has a sonic personality that comes from the room it was captured in, the mics chosen, the way the engineer placed them, and the post-processing applied. Ownhammer leans dry, surgical, and dense, perfect for studio direct recording where the tone needs to fit cleanly into a mix. York Audio leans warm, three-dimensional, and slightly compressed, better for live FRFR where you want the IR to do some of the room-feel work the cab can't. ML Sound Lab leans bright, focused, and modern-metal-coded, designed to cut through dense djent mixes without EQ surgery. CelestionPlus is the official Celestion-licensed library, conservative and reliable across genres. Wilkinson Audio's free God's Cab pack and Celestion's free sampler IR are legitimately good and cover most use cases without spending anything. The right library depends on what you're trying to do; there's no single best, only better fits for specific signal chains.

| Library | Price | Best For | Voicing | Library Size |
|---|---|---|---|---|
| OwnHammer | $20-90 per pack | Studio direct recording, mix integration | Dry, dense, surgical | ~100 cabs |
...
| CelestionPlus | $12-140 per cab/collection | Reliable defaults across genres | Conservative, true-to-cab | ~50 cabs |
| God's Cab (Wilkinson Audio) | Free | Anyone starting out | Accurate Mesa oversized 4×12 | 700+ IRs |
| Celestion free IR | Free (email signup) | A known-good baseline | True-to-Celestion-speaker | 1 IR mix |
...
The library is huge. The 412 range alone has 30+ cab options across Mesa, Marshall, Bogner, and Engl variants. The 212 range covers Vox AC30, Fender Deluxe, Vox AC15, and other smaller-format cabs. The 112 range is essential for boutique amp users. Buy the packs you'll actually use; the all-cab bundle is more than most players need.
...
<GearPick
  name="IR Library"
  manufacturer="Ownhammer"
  verdict="The studio-direct default. Dry, surgical, mix-ready."
/>
...
<GearPick
  name="IR Library"
  manufacturer="York Audio"
  verdict="Warm and three-dimensional. The FRFR-flattering library."
/>
...
The library includes specialty packs that go deep on specific cabs (Mesa Recto Standard, Bogner Uberkab, Engl Pro 4×12) and signature packs from artist collaborations like the Misha Mansoor series. The artist packs are especially good for players trying to nail a specific tone; they're effectively tone-recipe IRs.
...
<GearPick
  name="IR Packs"
  manufacturer="ML Sound Lab"
  verdict="Modern-metal specialist. Present-mid cut that wins in dense mixes."
/>
...
The pricing model is per-cab (or per-collection) rather than per-library, which makes it expensive if you want broad coverage but reasonable if you want only one or two specific cabs. The G12M-25 Greenback cab IR alone is about $12 and worth it for any Marshall-style modeler preset.
...
<GearPick
  name="CelestionPlus IRs"
  manufacturer="Celestion"
  verdict="Manufacturer-licensed accuracy. The scientifically clean A/B option."
/>
...
**God's Cab (Wilkinson Audio).** Over 700 IRs of a Mesa oversized Recto 4×12 with V30s, free to download. The pack covers the most-used cab in modern rock and metal, and the mic options span an SM57, an SM7B, an MD421, and large-diaphragm condensers at multiple positions. For anyone starting out, this pack alone covers 70% of the use cases a paid Mesa-cab library would.

**Celestion's free IR.** Celestion offers a genuine CelestionPlus impulse response free (the Cenzo Townshend mix, in exchange for a mailing-list signup). It's a single IR rather than a library, but it's captured to the same standard as the paid CelestionPlus catalog and makes a useful known-good baseline.

Most players who tell me they "can't afford" cab IRs haven't tried these two free options. Try them first. If you find yourself wanting more variety or specific non-Mesa cabs, then spend money on the paid libraries.
...
**Practice through headphones, no mix context:** York Audio or the free God's Cab pack. Either one gives you a pleasant tone for solo playing.
...
**Genre-spanning bedroom rig:** God's Cab + Celestion's free IR. Total spent: zero. Coverage: most of what you'll ever need.
...
2. **Pick three IRs from the library you're auditioning:** one close-mic'd 57, one ribbon mic option, one mid-room or larger-distance mic. This gives you the spread of mic perspectives, and the spread is real — a 57 bites, a ribbon smooths the top, a condenser opens up the air, exactly as they do on a live cab. If those differences aren't intuitive yet, our [SM57 vs. ribbon vs. condenser breakdown](/blog/sm57-vs-ribbon-vs-condenser-guitar-cab-which-mic) walks through what each mic does to the tone, which is the same character you're hearing baked into these captures.
...
For a deeper look at how the FRFR cab affects the IR choice, our [Friedman ASM-12 vs. Atomic CLR comparison](/blog/friedman-asm-12-vs-atomic-clr-mk-ii-frfr) walks through the powered-cab side of this same question, and our [stock Helix cabs vs. third-party IRs piece](/blog/helix-ir-shootout) covers when an upgrade to a paid library is actually worth the money.
...
I've also kept a handful of the free God's Cab IRs on my Quad Cortex's internal storage as a backup. If I ever lose my paid IRs in a firmware update or a project file corruption, I can fall back to the free pack and finish the session. The free pack is good enough for that.
...
<SaveThisTone title="The cab-IR-tested preset bundle" description="Our preset library includes Helix and Quad Cortex presets dialed against the free God's Cab pack and the York Audio Mesa Standard pack. Known IRs, documented mic positions, A/B-able to your own library." href="/recipes/cab-ir-tested-presets" ctaLabel="Get the IR-tested preset bundle" />
