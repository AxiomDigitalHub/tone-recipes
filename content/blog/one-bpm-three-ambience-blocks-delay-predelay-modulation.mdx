---
title: "One BPM, Three Ambience Blocks: Delay, Predelay, and Modulation on a Single Grid"
description: >-
  Lock delay time, reverb predelay, and modulation rate to the same tempo and the
  whole ambient bed moves as one. The note-division math, a worked example, and
  how to set it on Helix and Quad Cortex.
date: '2026-06-07'
...
takeaways:
  - >-
    ...
    and modulation rate. When all three derive from the same BPM, the ambient bed
    pulses as a single organism instead of three effects fighting the groove.
  - >-
    Time-based values come from one formula — milliseconds equal 60000 divided by
    BPM, times the note division. Modulation is a rate, not a time, so it uses
    BPM over 60 divided by the beats per LFO cycle to get hertz.
  - >-
  ...
  - >-
    ...
    announces itself.
  - >-
    On Helix and Quad Cortex you can set delay and (on most blocks) modulation to
    note values directly. For predelay, which is usually a millisecond field,
    compute the 32nd-note value from the formula and type it in.
faq:
  - q: How do I sync modulation to tempo on guitar?
    a: >-
      Modulation rate is a frequency, so convert tempo to hertz: take BPM divided
      by 60 to get beats per second, then divide by how many beats you want per
      LFO cycle. One cycle per bar in 4/4 at 84 BPM is 84/60 divided by 4, which
      is about 0.35Hz. Many modeler modulation blocks also accept tempo-synced
      note values directly, in which case you just pick the note division.
  ...
  - q: Can I sync modulation to tempo on Helix and Quad Cortex?
    a: >-
      ...
      tempo-synced note value rather than a fixed hertz figure. Delay blocks sync
      the same way. The one parameter that's usually a plain millisecond field is
      reverb predelay, so for that you compute the note value in milliseconds and
      enter it manually.
---
