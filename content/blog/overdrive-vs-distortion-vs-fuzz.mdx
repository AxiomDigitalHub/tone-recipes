...
takeaways:
  - >-
    The three dirt types are separated by how they clip the waveform: overdrive
    softly rounds the peaks (touch-sensitive, amp-like), distortion hard-clips
    them flat (saturated, consistent), and fuzz squares the whole wave off
    (raw, primal, transistor-driven).
  - >-
    Overdrive follows your hands — it cleans up when you roll back the guitar
    volume. Distortion holds a more constant gain regardless of pick attack.
    Fuzz is the least dynamic and the most reactive to the guitar's volume knob.
  - >-
    A Tube Screamer adds little gain on its own; it pushes the amp's preamp and
    adds a midrange bump. That's why drive pedals belong in front of the amp,
    not in the effects loop.
  - >-
    Use less gain than you think you need. Excess gain blurs note definition and
    buries chords in fizz — especially on a modeler, where gain is essentially
    free. If you can't hear each note in a chord, back it off and turn up the
    level.
faq:
  - q: What's the actual difference between overdrive, distortion, and fuzz?
    a: >-
      The clipping architecture. Overdrive uses soft clipping that gently rounds
      the waveform peaks, emulating a pushed tube amp and staying touch-
      sensitive. Distortion uses hard clipping that chops the peaks flat for a
      saturated, more consistent gain. Fuzz uses transistor saturation that
      squares off the entire waveform for a raw, buzzy, primal sound. Each
      produces a different overtone profile and a different feel under the
      fingers.
  - q: Which one cleans up when I roll back my guitar's volume?
    a: >-
      Overdrive cleans up the most predictably — it's the soft-clipping
      character that makes it feel like the amp opening up, so backing off the
      volume knob takes it from crunch toward clean. Fuzz also reacts strongly to
      the guitar volume (often more dramatically), while distortion holds a more
      constant amount of gain regardless of how hard you pick.
  - q: Should a drive pedal go in front of the amp or in the effects loop?
    a: >-
      In front, almost always. A drive pedal's job is to push the preamp, and
      that only happens at the front input where it interacts with the amp's
      gain and tone stack. In the effects loop it distorts an already-EQ'd
      signal the amp's knobs can't shape, which usually sounds harsh and fizzy.
  - q: How much gain should I use?
    a: >-
      Less than you think. The best dirt tones have enough gain to sustain and
      add overtones but not so much that individual notes blur together. A good
      test: if you can't clearly hear each note in a chord, there's too much
      gain. Back it off and raise the level to compensate.
---
...
For more on drive placement and the full effect order, see our [complete guide to signal chain order](/blog/signal-chain-order-guide). And because a drive pedal's whole job is to push the preamp, *where* you plug it in changes everything — our breakdown of [overdrive in the effects loop vs. the front of the amp](/blog/overdrive-in-effects-loop-vs-front-of-amp) explains why dirt out front sings and dirt in the loop buzzes.
