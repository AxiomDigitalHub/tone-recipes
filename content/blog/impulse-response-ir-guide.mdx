...
seo_title: "Impulse Responses Explained: What IRs Are"
...
image_alt: "A close microphone on a guitar speaker cabinet beside a laptop showing an impulse response file loaded into modeler cab-simulation software"
takeaways:
  - The speaker cab and mic — frozen as an impulse response — account for roughly half of what you hear as "amp tone," which is why swapping the IR changes a sound more than swapping the amp model.
  - An IR is a short WAV file (about 20 ms) that captures one cab, one mic, one position; your modeler convolves your live signal with it in real time to filter it like that speaker.
  - Stock IRs are voiced to be safe and versatile; a well-made third-party pack is usually the single biggest dollar-per-tone upgrade a modeler player can make.
  - Load IRs as mono WAV at your modeler's internal rate (usually 48 kHz). Blending two IRs only works cleanly when they're phase-coherent captures from the same pack, or you get hollow cancellation.
  - For live full-range use, stick to close-mic IRs with no room component — the room-mic and blended IRs that shine in a mix clash with the real room you're standing in.
faq:
  - q: What's the difference between an IR and a cab model?
    a: >-
      A cab model is an algorithmic simulation — the modeler uses math to
      approximate how a speaker behaves. An IR is a measured capture of a real
      speaker's actual behavior. Both simulate a cabinet, but IRs capture the
      specific character of one physical cab. Many modelers support both and let
      you swap freely between them in the same preset.
  - q: Can I use the same IR on different modelers?
    a: >-
      Yes. A WAV IR file is a universal format. The same IR loaded on a Helix,
      Quad Cortex, Kemper, or Fractal produces the same tonal result, assuming
      the modeler handles the sample rate correctly. Your cab tone is portable
      across platforms — one of the best things about working with IRs.
  - q: Do IRs work with real tube amps?
    a: >-
      Yes, but you need hardware to run them. A reactive load box such as the Two
      Notes Captor X, Suhr Reactive Load IR, or UA OX sits between your amp's
      speaker output and a load or speaker, and applies the IR for recording or
      silent playing. That gives you IR cab simulation with the feel of a real
      tube amp. A standalone IR loader pedal does the same for a pedalboard with
      no amp at all.
  - q: How many IRs do I actually need?
    a: >-
      Most working guitarists settle on three to five: one for
      clean/edge-of-breakup, one for crunch rhythm, one for high-gain, and maybe
      one or two specialty options for recording. Start small, learn what you
      like, and expand only when you hit a sound your current set can't reach.
  - q: Should I use IRs or my modeler's built-in cab models?
    a: >-
      Try both. Some modelers have excellent built-in cab models that rival
      third-party IRs; others have a cab section that's clearly the weakest link.
      Load a well-reviewed third-party IR next to the stock cab model for the
      same speaker type and compare. If the stock cab sounds good to you, there's
      no reason to change. If the IR is noticeably better, you've found your
      biggest upgrade path.
---
...
Here's the part that surprises people: the speaker cabinet contributes roughly 50% of what you recognize as "amp tone." Not the preamp tubes, not the power section, not the circuit topology. It's the wooden box with a cone in it and the microphone you point at it.
...
## No Modeler? Load IRs in a Pedal

Everything above assumes you own a modeler with a cab or IR block. You don't have to. A standalone IR loader pedal does the convolution in a small box you put at the end of an otherwise all-analog pedalboard — so a rig built around real overdrives and a preamp pedal can go straight to a PA or interface with a miked-cab sound and no amp in the room. The IR file is the same WAV; only the thing playing it back changes. If your signal chain is pedals rather than a Helix or Quad Cortex, the [IR loader pedal guide](/blog/ir-loader-pedals-direct-rig-two-notes-boss-ir2-mooer) covers the hardware (Two Notes, BOSS, Mooer, UAFX) and the one rule that decides whether a direct rig sounds finished — cab sim on for full-range, off into a real power amp.
