...
seo_title: "HX Stomp Polarity Trick: Two Cabs Out of Phase"
description: >-
...
Configure your amp model as normal. The cab polarity trick applies downstream of the amp, so the amp settings don't need to change.
...
|---|---|---|---|---|---|
| Amp | (none) | (none) | Effects (reverb/delay) | Cab A | Cab B (inverted) |
| Compressor | Amp | (none) | Effects | Cab A | Cab B (inverted) |
| Drive | Amp | (none) | Effects | Cab A | Cab B (inverted) |
...
The most common problem with the polarity trick is over-cancellation, where too much of the fundamental is being removed. Usually one of three causes:
...
On full Helix, dual-cab mode in a single block blends two cabs in phase with no polarity inversion. The result is an average of the two cabs' tones. The polarity trick emphasizes the *differences* between the cabs rather than blending them. Dual-cab mode is simpler and works well for standard cab blending; the polarity trick produces a specific character that dual-cab mode doesn't replicate.

<SaveThisTone
  title="Every recipe ships a HX Stomp preset"
  ...
  ctaLabel="Browse HX Stomp recipes"
/>
