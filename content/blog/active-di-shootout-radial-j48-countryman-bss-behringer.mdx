...
title: >-
  ...
  Behringer DI100
seo_title: "Active DI Shootout: Radial J48 to Behringer DI100"
description: >-
...
image_alt: >-
  Four active DI boxes lined up on a studio desk — Radial J48, Countryman Type
  85, BSS AR-133 and Behringer DI100 — XLR cables running off to a patchbay
takeaways:
  ...
  - "The Countryman Type 85 ($219) is the live FOH standard and has by far the highest input impedance here — Countryman specs 10 MΩ in pickup mode — which is why it is the box to reach for when a passive pickup plugs straight in."
  - "The BSS AR-133 ($249) pairs a 1 MΩ input with a 40 dB pad that handles amp-level signals, but the pad drops the input to 47 kΩ — you get the high impedance or the pad, not both at once."
...
faq:
  - q: "When should I use an active DI instead of a passive one?"
    a: "Active DIs are better for high-impedance sources — passive guitar pickups, piezo systems, and electric pianos — because they present a higher input impedance (220 kΩ to 10 MΩ across the boxes here) that preserves the source's high-frequency content. Passive transformer DIs present a far lower input impedance: the Radial JDI is 140 kΩ and the Whirlwind IMP 2 is around 100 kΩ, both well under the 1 MΩ a guitar amp input gives you. That is fine for line-level modeler outputs but audibly dulls a passive pickup. The rule: if your source is a battery-powered preamp or a line-level output, a passive DI is simpler and quieter. If your source is a passive pickup or any high-impedance signal, an active DI preserves more of the signal."
...
| Countryman Type 85 | -114 | ±1.0 (1 kHz +1 dB rise) | 10 MΩ (pickup mode) / 10 kΩ (speaker mode) | +12 | 48V phantom required | $219 |
| BSS AR-133 | -113 | ±0.8 | 1 MΩ (pad at 0 dB) / 47 kΩ (pad engaged) | +12 (no pad), +52 (40 dB pad) | 48V phantom or internal 9V battery | $249 |
...
Input impedance is where active DIs do their actual work. A passive guitar pickup wants to see at least 250 kΩ at its output for the full frequency response to come through; load it with less than that and the high end rolls off, because the load damps the pickup's resonant peak. The spread across these four boxes is much wider than the price spread.

The Countryman is the outlier, by a factor of ten. Countryman specifies 10 MΩ in pickup mode — ten times a typical guitar amp input — which makes it the least intrusive load of anything here by a wide margin. (Its speaker mode is a different circuit at 10 kΩ; that mode is for amp speaker taps, not for a guitar.) The BSS follows at 1 MΩ with its pad at 0 dB, roughly an amp input's worth of load, though engaging the 20 or 40 dB pad drops the input to 47 kΩ — so the AR-133's headroom trick and its pickup-friendliness are two settings, not one. The J48 at 220 kΩ is at the lower edge of what leaves a passive pickup unaffected, and the DI100 also claims 220 kΩ.

**Correction, 2026-09-15:** an earlier version of this post listed the Type 85 at 100 kΩ and built a top-end rolloff claim on that figure. The spec is 10 MΩ in pickup mode, and that claim is withdrawn. The Type 85 is the highest-impedance box in this comparison, not the lowest — which is the actual technical reason it became the front-of-house standard for plugging a passive instrument straight in.
...
The 10 MΩ input impedance in pickup mode is the Type 85's headline spec and the reason it earned its place on FOH inputs: a passive pickup or a piezo plugged straight into it is loaded less than it would be by any guitar amp. The trade-off list is elsewhere — no battery option, phantom required, and a fixed +12 dBu input ceiling with no pad on the instrument side. Use the separate speaker-mode input for an amp tap; that path is 10 kΩ and is not where a guitar goes.
...
The 1 MΩ input impedance is roughly what a guitar amp's input gives you, and it is a meaningfully lighter load than the J48's 220 kΩ. The caveat is in BSS's own spec: that 1 MΩ applies with the pad at 0 dB, and engaging the 20 or 40 dB pad drops the input to 47 kΩ. For a passive pickup straight in, leave the pad off. For a speaker tap or a hot line source, use the pad and accept that the impedance advantage is gone — you don't need it there anyway.
...
**Best for:** Broadcast and theater installs, line-level sources that need padding (synth keyboards, modeler outputs, amp speaker taps via the 40 dB pad), and passive pickups when the pad is off.
...
For direct passive-guitar recording — no amp, no modeler, just a guitar cable into the DI — the active DI wins on input impedance, and the order here is not the order the prices suggest: Type 85 at 10 MΩ, AR-133 at 1 MΩ with the pad off, then the J48 and DI100 at 220 kΩ. All four beat a passive transformer DI for this job.

This matters more than it sounds if the DI is going to be a reamp source, because whatever the input did to your pickup is frozen into the file before any comparison starts — [which is the part of reamping that nothing downstream can undo](/blog/reamp-comparisons-what-they-cant-test).

## Which One Should You Buy?

The four boxes serve four different jobs.

**Studio active DI for direct passive-guitar recording:** Radial J48 if you want the discrete-transistor topology and the smallest noise floor; BSS AR-133 if you want the lighter load on the pickup and the bigger pad range, remembering that you only get the 1 MΩ with the pad off. If the *only* thing you care about is loading the pickup as little as possible — a piezo, a low-output vintage single coil, a DI you plan to reamp from for years — the Type 85's 10 MΩ beats both, and you give up the pad and the battery option to get it.

**Live FOH active DI:** Countryman Type 85. Period. The form factor, the bombproof build, the slight presence rise, and the 10 MΩ input that lets any passive instrument plug straight in without a buffer — all four are designed for live work in a way the studio DIs aren't.
