...
<FAQ questions={[
  {
    ...
    a: "Sometimes. An active modeler with a balanced XLR output (like the Helix or the Quad Cortex) doesn't strictly need a DI for level conversion; the XLR output is already mic-level and balanced. But a transformer DI breaks the ground loop between the modeler's interface and the FOH console, which is the most common reason guitarists need one. If you're running an unbalanced ¼″ output to FOH and getting hum, a DI fixes it. If you're running balanced XLR direct to FOH and not getting hum, you don't need a DI. If you're getting hum despite the balanced XLR, you have a different ground-loop problem and a DI may or may not fix it depending on the source."
  },
  {
    q: "Why is the Countryman Type 85 here if it's active and the rest are passive?",
    a: "Because players actually cross-shop these. The Type 85 is the live-FOH standard the way the JDI is the studio standard, and the comparison most buyers want is 'which one for live?' The active topology gives the Type 85 a higher input impedance that's better for passive pickups, which is the technical reason it's the FOH standard for acoustic guitar. The passive boxes (JDI, IMP 2, CleanBox) are better for active outputs like modelers because they don't need phantom and they have more headroom. The active vs. passive distinction is the whole story of why the Type 85 is here."
  },
  {
  ...
  },
  {
    ...
    a: "You can, but the JDI and Whirlwind have an input impedance (140 kohm and 100 kohm respectively) that's lower than a typical guitar amp's 1 megohm input. This loads down passive single-coil pickups slightly and reduces the high-frequency content of the signal. The Countryman Type 85's 10 megohm input impedance avoids this issue, which is why it's the FOH standard for acoustic guitar with passive piezos. If you absolutely need to run a passive guitar directly into a passive transformer DI, expect a slight loss of high-end clarity. A buffer pedal or a modeler in front of the DI fixes this."
  },
  {
    q: "Does it matter which side of the stage the DI sits on for ground-loop fixes?",
    a: "Yes, slightly. The DI breaks the ground at its location in the chain. If the ground loop is between your pedalboard's power supply and the FOH console, putting the DI right after your pedalboard fixes it. If the ground loop is between your guitar and the modeler (less common), the DI needs to sit before the modeler. In practice, the standard placement (last in the chain, just before the cable run to FOH) catches the most common ground-loop sources and is the right default. If you have a stubborn loop that doesn't go away with the DI in the standard position, try moving the DI earlier in the chain to isolate where the loop is actually originating."
  }
]} />
