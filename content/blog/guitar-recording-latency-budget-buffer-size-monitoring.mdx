...
seo_title: "Your Latency Budget: Buffer Size and Round-Trip Lag"
description: >-
...
## Sample Rate Is Not the Second Lever

Someone always asks the obvious next question: if buffer size controls latency, doesn't a higher sample rate control it too? On paper, yes — 128 samples represents less time at 96 kHz than at 48 kHz, so the buffer's share of the delay shrinks. But look at what you just read. The buffer share isn't the problem; the fixed conversion-and-driver floor is, and raising the sample rate doesn't touch it. So the round-trip drops by a millisecond or two while your CPU load roughly doubles, because every plugin now processes twice as many samples per second. You spend half your plugin headroom to save what the buffer floor won't give back anyway. The sample rate is a tone decision — aliasing, high-gain top end — not a latency decision. I broke that math down in full in [does 96 kHz actually lower your latency](/blog/does-96khz-lower-latency-sample-rate-buffer-size). The short version: use the buffer for latency, and pick a sample rate for reasons that have nothing to do with it.
