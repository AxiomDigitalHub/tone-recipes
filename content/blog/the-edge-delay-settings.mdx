...
The secret is the **dotted eighth note delay.** While most delay effects are set to quarter notes or straight eighth notes (repeating right on the beat), a dotted eighth delay is slightly longer — exactly 75% of one beat. That offset timing means the delay repeats land *between* the beats, interlocking with the dry notes to create a cascading, shimmering rhythm.
...
**Use less gain than you think.** The Edge typically plays with a light crunch or even clean tones. Heavy distortion compresses the signal and makes delay repeats smear together into something indistinct. Keep gain moderate so each note and each delay repeat stays articulate and clear.
