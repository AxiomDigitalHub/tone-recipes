...
<FAQ questions={[
  ...
  },
  {
    ...
    a: "Not in any way most players can hear. The tuner's mass and post material can theoretically affect the headstock's resonant behavior; heavier tuners damp the headstock slightly, which can result in marginally faster note decay. In practice, this difference is below the threshold of audibility on a recorded track. The exception is a featherweight semi-hollow guitar where the headstock-to-body mass ratio is unusual; in that case, switching from heavy Schallers to lighter Sperzels or Hipshots can subtly change the playing feel without changing the recorded tone."
  },
  {
    q: "Can I install locking tuners on a guitar with vintage 10mm bushing holes?",
    a: "Yes. All four brands tested ship with adapter bushings that fit vintage 10mm bushing holes (the standard for pre-CBS Fenders and most modern reissues). No drilling or hole modification is required. The screw mount locations may differ slightly from your existing tuners; pre-drill new pilot holes with a 1/16″ bit if needed to prevent the wood from splitting."
  },
  {
    q: "What's the difference between staggered and non-staggered locking tuners?",
    a: "Staggered tuners have different post heights for different strings (lower posts for the high strings, taller posts for the low strings), which produces more uniform string-break angle behind the nut. This is what string trees on a vintage Strat are trying to solve. By using staggered tuner posts, you can eliminate string trees entirely. Sperzel Trim-Lok is naturally staggered. Hipshot Grip-Lock comes in both staggered and non-staggered versions. Schaller and Gotoh are typically non-staggered (you keep the string trees). For a vintage-style Strat, staggered tuners give you a cleaner setup with no string trees needed."
  },
  {
