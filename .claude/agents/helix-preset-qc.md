...
13. **Dual-mic cab via `cabSibling` — `cab0` has minimal shape, `@cab` ref goes on the in-chain cab (not the amp)** — recipe blocks declare `cabSibling: { Mic, Position, ... }` to opt into a dual-mic blend. The factory corpus has TWO distinct dual-cab patterns:
    - **Pattern A** (130 cases): amp has `@cab: "cab0"`, no in-chain cab block, cab0 sibling alone. Single-mic via amp-attached cab.
    - **Pattern B** (66 cases, what we use for dual-mic): amp has NO @cab reference, in-chain cab block has `@cab: "cab0"`, cab0 sibling carries the second mic. True dual-mic blend.

    For Pattern B, the generator (a) auto-promotes the cab model ID to its WithPan variant via `withPanVariant()` lookup, (b) emits the in-chain cab as `@type: 4` with `@cab: "cab0"`, and (c) emits a `cab0` sibling object on the SAME DSP as the in-chain cab.

    **The `cab0` sibling has a minimal shape** — only `@model` and `@enabled` as `@`-prefixed metadata, plus the param fields (Mic, Position, Pan, Angle, Delay, LowCut, HighCut, Level, Distance for WithPan). It DOES NOT have `@position`, `@type`, `@path`, `@no_snapshot_bypass` — including any of those breaks HX Edit's import. **The `@cab: "cab0"` reference belongs on the in-chain cab block, NOT on the amp.** Putting it on the amp breaks Pattern B.

    Topology: amp on dsp0 @position 7, in-chain cab + cab0 sibling both on dsp1 @position 0, time-based effects after them on dsp1. Cross-DSP routing carries the signal from dsp0 → dsp1.

    WithPan cabs accept the full param set (Mic, Position, Pan, Angle, Delay, plus the 5 legacy params); legacy `@type: 2` cabs only accept the 5 legacy params. (Implemented + thoroughly corrected 2026-04-26 after factory-corpus survey.)
