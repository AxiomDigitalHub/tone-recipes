...
13. **Dual-mic cab via `cabSibling` recipe field — REQUIRES split-DSP topology with cab on dsp1** — recipe blocks declare `cabSibling: { Mic, Position, ... }` to opt into a dual-mic blend. The generator (a) auto-promotes the legacy cab model ID to its WithPan variant via `withPanVariant()` lookup (`HD2_Cab<X>` → `HD2_CabMicIr_<X>WithPan`), (b) emits the in-chain cab as `@type: 4`, (c) emits a sibling `cab0` object on the SAME DSP as the in-chain cab (same model, same position), and (d) adds `@cab: "cab0"` to the amp block as a cross-DSP UI hint. **The cab + cab0 sibling must live on dsp1, NOT alongside the amp on dsp0.** Splitting them onto the same DSP as the amp makes HX Edit reject the preset on import (the file looks structurally valid but won't open). Verified-working topology: amp on dsp0 @position 7, cab + cab0 on dsp1 @position 0, with time-based effects after them on dsp1. The generator's split logic now detects `hasDualMicCab` and forces split-after-amp for these chains. WithPan cabs accept the full param set (Mic, Position, Pan, Angle, Delay, plus the 5 legacy params); legacy `@type: 2` cabs only accept the 5 legacy params. (Implemented + corrected 2026-04-26.)
