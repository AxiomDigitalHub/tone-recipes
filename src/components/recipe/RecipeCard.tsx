...
export default function RecipeCard({ recipe, artist, song }: RecipeCardProps) {
  ...
  return (
    <Link
      ...
      className="card-hover group relative flex flex-col overflow-hidden rounded-xl border border-[var(--ink)]/15 bg-[var(--paper-2)] transition-[border-color,background-color,box-shadow] duration-200 hover:border-[var(--amber)]/40 hover:bg-[var(--paper)] hover:shadow-lg hover:shadow-black/20"
    >
      {/* Album art header */}
      {song?.album_art_url ? (
        <div className="relative h-40 w-full overflow-hidden bg-[var(--ink)]/30">
          <Image
        ...
        <div className="h-3 w-full bg-gradient-to-r from-[var(--amber)]/20 to-[var(--amber)]/5" />
      )}
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mb-3">
          <div className="flex items-center gap-1.5">
            <h3 className="text-base font-semibold text-[var(--ink)] group-hover:text-[var(--amber-2)] transition-colors line-clamp-1">
              {song ? song.title : recipe.title}
            </h3>
            <VerificationBadge level={verificationLevel} size="sm" />
          </div>
          {!song?.album_art_url && artist && (
            <p className="text-sm text-[var(--ink-muted)]">{artist.name}</p>
          )}
        </div>

        {/* One-line description */}
        {recipe.description && (
          <p className="text-xs text-[var(--ink-muted)] line-clamp-1 mb-3">
            {recipe.description}
          </p>
        )}
        ...
        <div className="flex items-center gap-1 overflow-hidden">
          {recipe.signal_chain.slice(0, 5).map((node, i) => {
            ...
            <div key={i} className="flex items-center gap-1">
              <div
                className="node-glow flex h-8 w-8 items-center justify-center rounded-md border border-[var(--ink)]/15 transition-all"
          ...
          {chainLength > 5 && (
            <span className="text-xs text-[var(--ink-muted)]">+{chainLength - 5}</span>
          )}
        </div>

        {/* CTA */}
        <div className="mt-auto pt-3">
          <span className="inline-flex items-center gap-1 text-xs font-medium text-[var(--amber-2)] group-hover:underline">
            Get this tone
