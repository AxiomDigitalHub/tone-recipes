...
function NodeIcon({ label, color, size = 28 }: { label: string; color: string; size?: number }) {
  switch (label) {
    case "GUITAR":
      // Rotated 45° and scaled so the guitar fills its tile the same
      // way it does in v2/v3's shared NodeIcon — the old 0.45/0.7
      // multiplier left it visibly smaller than every other glyph.
      return (
        <svg
              ...
              <React.Fragment key={node.label}>
                <div
                  ...
                  style={{
                    ...
                    boxShadow: isLit
                      ? `0 0 0 1px ${node.color}15, 0 8px 28px ${node.color}${isCurrent ? "60" : "30"}`
                      : "none",
                  }}
                >
                  <NodeIcon label={node.label} color={isLit ? node.color : "#2a3a55"} size={29} />
                  <div style={{
