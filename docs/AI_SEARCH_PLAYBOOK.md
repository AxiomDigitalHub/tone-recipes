...
> **✅ CLOUDFLARE-ERA CONFIG CONFIRMED 2026-07-23 (dashboard-verified):** Bot Fight Mode **OFF** (was the sole challenger of PerplexityBot — 48 challenges/day while on; Perplexity is de-verified by Cloudflare so no exemption covered it). AI Crawl Control per-crawler blocks: none. AI bot policies: Search/Agent/Training all Allow. "Block AI bots" managed rule: off. AI Labyrinth: off. Managed robots.txt: off (our robots.ts serves). **Standing rule:** these switches stay in this state; if attacks force Bot Fight Mode back on, accept that de-verified crawlers (Perplexity) will be blocked, or upgrade to Pro for exception support. "Markdown for Agents" toggle is Pro-plan-gated — noted as blocked-by-plan, not declined. **Monthly check:** export Security → Events (24h JSON) and grep for AI-bot UAs with `managed_challenge`/`block` — laptop curls prove nothing (residential IPs aren't challenged; crawler datacenter IPs are the real test). Awaiting one post-change export to confirm zero PerplexityBot challenges.
>
...
| OAuth discovery: RFC 9728 /.well-known/oauth-protected-resource + WWW-Authenticate on API 401s (2026-07-25) | ✅ Shipped honest version — F&K is a *resource server*, Supabase is the issuer; metadata points at `$NEXT_PUBLIC_SUPABASE_URL/auth/v1` (which serves its own OIDC discovery + JWKS). /.well-known/openid-configuration 307s to Supabase's discovery doc rather than faking an issuer here (would fail RFC 8414 issuer validation). No dynamic client registration on the Supabase side yet — revisit if that or a real MCP endpoint ships (ToneTrace era) |
| auth.md agent registration (workos.com/auth-md, 2026-07-25) | ✅ Adopted honestly — /auth.md documents the flow that actually exists and was verified live end-to-end: agents CAN self-serve accounts+tokens (GoTrue `/signup` returns a session, confirmations off; `/token?grant_type=password`; Bearer JWT works on /api/tone-chat). agent_auth block served in both /auth.md and /.well-known/oauth-authorization-server; the latter now serves accurate metadata instead of 307ing, because Supabase's own OIDC doc advertises `/oauth/*` endpoints that are feature_disabled for this project (dead-endpoint problem). scopes_supported honestly `[]`. Still no DCR/ID-JAG/anonymous — auth.md says so explicitly |
...
| DNS-AID `_agents` SVCB records (2026-07-23, revised 2026-07-25) | ✅ Adopted — the fictional-service objection dissolved: Axl (`/api/tone-chat`) plus the new read-only MCP server (`/api/mcp`, no auth, static corpus) are real agent endpoints. `_index._agents` + `_mcp._agents` SVCB published in Cloudflare pointing at `/.well-known/agents.json`; DNSSEC live (DS 2371 at GoDaddy, chain validated). Still an individual draft with ~no consumers — same zero-maintenance tier as llms.txt, not an AI lever. MCP server doubles as real distribution (usable as a Claude connector) |
