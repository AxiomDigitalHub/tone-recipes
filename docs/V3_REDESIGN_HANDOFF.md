...
**Status:** Design is locked-in directionally. Through v3.11: SEO/JSON-LD parity in place, /news + /request ported, 301 redirects staged commented in `next.config.ts`, count-flex eyebrows scrubbed sitewide, blog overview restructured (Departments sidebar + flat ledger), and a first-cut /preview/dashboard surface (Overview + Saved). Cutover is now a one-decision flip (URL strategy) plus uncommenting the redirects + verifying OAuth redirect URIs in Supabase + Google Cloud Console.
