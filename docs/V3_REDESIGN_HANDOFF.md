...
1. **Verify OAuth redirect URIs** in Supabase (Auth → URL Configuration) and Google Cloud Console (OAuth 2.0 Client). Same-domain swap means no code change, but confirm no Vercel-preview URLs leaked into the allowed list.
2. **Logged-in chrome on the sub-nav** — currently the sub-nav always shows "Log in / Sign up". Read `useAuth().user` and swap to an avatar dropdown when authed.
3. **Re-skin the dashboard inner pages** (`/dashboard/my-gear`, `/my-recipes`, `/notifications`, `/settings`, `/admin`) — the v3 sidebar + identity tile wraps them now, but the inner page content is still production-styled tailwind. Mechanical port using the v3 design tokens.
...
| v3.9 | 6ab991e | SEO + last-page-gap. `src/lib/seo/preview-jsonld.ts` helpers + JSON-LD on every detail route; real `<Metadata>` titles/descriptions/OpenGraph across all v3 routes (still `noindex`); 301 redirects staged commented in `next.config.ts`; `/preview/request` (split into `page.tsx` + `RequestClient.tsx`) and `/preview/news` (+ `/news/[slug]`) ported from production with paper/ink chrome. Sub-nav adds News + Request. Also: `scripts/validate-mdx.mts` MDX preflight + `f9dcf4c` MDX-prerender hotfix. |
| v3.10 | e89c02e | Daniel review: drop count-flex eyebrows ("X recipes translated", "X stories filed", "X songs · X recipes", About-page stat tiles). Hide platform-photo placeholder until real photography drops in (`platform-head-solo` single-col layout). `text-wrap: pretty` on `.platform-tagline` fixes the "in." orphan. Cutover blocker #7 added — verify Supabase + Google OAuth redirect URIs; no code change at cutover. Saved a durable feedback memory so the count-flex pattern doesn't creep back. |
| v3.11 | b8aea4c | Blog overview redesign + first cut at the logged-in surface. Departments → sticky left sidebar (vertical list, no `§` marks). Archive → flat newest-first 3-col ledger (Dept. / Title / By; volume-break headers + NO./FILED cols + `{n} filings` count meta dropped). Masthead colophon drops the "writers on staff" count meta. **New /preview/dashboard surface:** sticky sidebar (Overview / Saved / My gear / My recipes / Notifications / Settings + Admin when role permits), identity tile (initials + tier pill + welcome serif). `/preview/dashboard` overview implements Continue-where-you-left-off hero + Saved rail + Recently-viewed rail using the production `useFavoritesStore` + `useRecentlyViewedStore`. `/preview/dashboard/saved` does the full saved-recipes grid. Inner pages (my-gear / my-recipes / notifications / settings / admin) are sidebar stubs — design language is set, port is mechanical. Dev preview unavailable in this CLI session; v3.10+11 visual verification deferred to Vercel. |
| v4.0 | (this round, 3 commits) | **Swap-in-place cutover.** Every `/preview/<route>` promoted to `/<route>` (home, recipe, song, artist, platforms, browse, blog, news, compare, gear, how-it-works, pricing, about, request, login, signup, privacy, terms, affiliate-disclosure, dashboard {layout, page, saved, DashboardShell}). Shared v3 components moved out of `src/app/preview/_components/` into `src/components/v3/`. Internal `/preview/<X>` hrefs rewritten to `/<X>` site-wide. `noindex` stripped from every promoted public route. Root layout (`src/app/layout.tsx`) replaces the production `<Header>` with the v3 masthead + sub-nav, wraps everything in `<div className="fk-preview">`, keeps Footer / LazySearchPalette / AuthProvider / analytics. Wildcard 301 from `/preview/:path*` → `/:path*` in `next.config.ts`. `src/app/preview/` directory deleted; stylesheet renamed `preview.css` → `src/app/v3.css`. tsc clean (after `.next` cache clear). Visual verification deferred to Vercel build. |
